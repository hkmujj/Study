...
namespace MMI.Communacation.Control.AppLayer.SendDataStrategy
{
    ...
    /// 包头只包括索引
    /// </summary>
    internal class TypeCDataSendStrategy : IDataSendStrategy
    {
        ...
        public TypeCDataSendStrategy(ICommunacationDataSender netService,
            ...
            IConfig config, INetProjectDataConfig projectDataConfig)
        {
            ...
            else if (m_NetDataConfig.NetOutputDataPackage.PackageCount >= 1)
            {
                ...
                {
                    SysLog.Error("NetDataConfig.NetOutputDataPackage.PackageCount = {0}, 目前只支持发送{1}包数据，剩下的数据发送都将被忽略 !!!!",
                ...
                for (var i = 0; i < packageCount; i++)
                {
                    ...
                    {
                        Head =
                            new SendPackageHeadWrapper() {PackageHead = {TypeC = SendDataTypeHelper.AllSendDataType[i]}}
                    };
