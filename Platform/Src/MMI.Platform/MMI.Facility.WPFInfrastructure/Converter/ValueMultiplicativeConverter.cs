...
namespace MMI.Facility.WPFInfrastructure.Converter
{
    ...
    /// 数据相乘
    /// </summary>
    public class ValueMultiplicativeConverter : IMultiValueConverter
    {
        /// <summary>
        ...
        {
            if (values.All(a => a != DependencyProperty.UnsetValue))
            {
                return values.Aggregate(1d, (current, value) => current * System.Convert.ToDouble(value));
            }
