...
namespace ES.Facility.PublicModule.Util
{
    ...
    public class DynamicInvokeHelper
    {
        ...
        public static object DynamicInvoke(Type type, string arg)
        {
            ...
            if (!Dictionary.ContainsKey(type) && !type.IsEnum)
            {
                return null;
            }

            var action = type.IsEnum ? Dictionary[typeof (Enum)] : Dictionary[type];

            // for enum.
            if (action.Method.GetParameters().Length == 2)
            {
                return action.DynamicInvoke(new object[] { arg, type });
            }
            return action.DynamicInvoke(arg);
        ...
        public static IDictionary<Type, Delegate> CreateDictionary()
        {
            ...
            dictionary.Add(typeof (Enum),
                new Func<string, Type, Enum>((p, t) =>
                {
                    try
                    {
                        return (Enum) Enum.Parse(t, p);
                    }
                    catch 
                    {
                        return default( Enum );
