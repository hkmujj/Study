...
CALL msbuild.exe "%ScriptsDir%\RegisterMMIPlatformLibrary.proj" /verbosity:normal /p:BinFolder="%BinDir%" /p:Register=%Register%
IF ERRORLEVEL 1 GOTO ERROR
...
ECHO.
ECHO RegisterMMIBinaries.bat:
ECHO Registers the MMI binaries folders as assembly folders.
ECHO Must be executed from the source folder
ECHO.
ECHO Usage: 
ECHO RegisterMMIBinaries.bat /?
ECHO RegisterMMIBinaries.bat [/q] [/u]
ECHO.
ECHO Options:
ECHO - /? : Show this help message
ECHO - /q : Do not pause when done
ECHO - /u : Remove the MMI binaries registration
ECHO.
