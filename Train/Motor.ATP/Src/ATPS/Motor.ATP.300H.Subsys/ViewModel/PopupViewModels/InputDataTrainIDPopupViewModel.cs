...
namespace Motor.ATP._300H.Subsys.ViewModel.PopupViewModels
{
    ...
    public class InputDataTrainIDPopupViewModel : DriverPopupViewModelBase
    {
        ...
        {
            TitleContent = PopupViewStringKeys.StringTitleInputingTrainId;
            ...
            if (rlt.DriverInputType == DriverInputInterpreterResult.InputType.Control)
            {
                ...
                switch (word)
                {
                    case DriverInputControlWord.Ok:
                        DriverInterface.ATP.SendInterface.SendTrainId(new SendModel<DriverDataModel>(new DriverDataModel(null,TrainId)));
                        break;
                    case DriverInputControlWord.Cancel:
                        DriverInterface.ATP.SendInterface.SendTrainId(new SendModel<DriverDataModel>(new DriverDataModel(null, TrainId),SendModelType.Cancel));
                        break;
