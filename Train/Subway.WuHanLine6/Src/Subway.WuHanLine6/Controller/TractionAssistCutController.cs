...
namespace Subway.WuHanLine6.Controller
{
    ...
    public class TractionAssistCutController : ControllerBase<TractionAssistCutModel>
    {
        ...
        {
            ViewModel.DisplayStr = $"电制动切除状态   MP1:{GetChar(ViewModel.Brake[0].IsCheck)} M1: {GetChar(ViewModel.Brake[1].IsCheck)} M2: {GetChar(ViewModel.Brake[2].IsCheck)} MP2: {GetChar(ViewModel.Brake[3].IsCheck)}";
