...
namespace CBTC.DataAdapter
{
    public abstract class DataAdapterBase : IDataListener
    {
        ...
        public virtual void OnFloatChangedImp(object sender, CommunicationDataChangedWrapperArgs<float> dataChangedArgs)
        {
            ...
            //目标距离
            dataChangedArgs.UpdateIfContains(InfKeys.目标距离, f => SignalDataIn.GoalDistance = f);


            //列车速度
            dataChangedArgs.UpdateIfContains(InfKeys.列车运行速度, f => SignalDataIn.TrainSpeed = f);
            ...
            //列车限制速度
            dataChangedArgs.UpdateIfContains(InfKeys.列车限制速度, f => SignalDataIn.TrainLimitSpeed = f);

            //列车推荐速度
            dataChangedArgs.UpdateIfContains(InfKeys.列车推荐速度, f => SignalDataIn.TrainRecommendSpeed = f);

            //列车目标速度
            dataChangedArgs.UpdateIfContains(InfKeys.列车目标速度, f => SignalDataIn.TrainGoalSpeed = f);

            //列车报警速度
            dataChangedArgs.UpdateIfContains(InfKeys.列车报警速度, f => SignalDataIn.TrainWarningSpeed = f);

            //列车紧急速度
            dataChangedArgs.UpdateIfContains(InfKeys.列车紧急速度, f => SignalDataIn.TrainEmergSpeed = f);
