...
namespace ES.Facility.PublicModule.Util
{
    ...
    /// </summary>
    [Serializable]
    public class XmlSerializationList<T> : List<T>, IXmlSerializable
    {
        static XmlSerializationList()
        {
            // TODO 
            XmlSerializationList<string>.Convertor = s => s;
            XmlSerializationList<int>.Convertor = Convert.ToInt32;
            XmlSerializationList<float>.Convertor = Convert.ToSingle;
            XmlSerializationList<double>.Convertor = Convert.ToDouble;
        }
        ...
        public XmlSerializationList(IEnumerable<T> collection)
            : base(collection)
        {
        ...
        public virtual void ReadXml(XmlReader reader)
        {
            ...
            {
                AddRange(GetListOfType(reader.Value));
    ...
    static class XmlSerializationListTest
    {
        ...
        {
            var ls = new XmlSerializationList<int>() { 1, 2, 3 };

            //DataSerialization.SerializeToXmlFile(ls, "D:\\a.xml", "rot");

            var data = new XmlSerializationListModel() { Datas = new XmlSerializationList<int>() { 1, 2, 3 } };
    ...
    {
        [XmlElement]
        public XmlSerializationList<int> Datas { set; get; }
    }
}