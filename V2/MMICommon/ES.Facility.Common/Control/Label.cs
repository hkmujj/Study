...
namespace ES.Facility.Common.Control
{
    ...
    public class Label : IControl
    {
        ...
        public void Paint(Graphics dcGs)
        {
            ...
            {
                dcGs.DrawRectangle(((LabelStyle)_style).BorderPen, _rect.X, _rect.Y, _rect.Width, _rect.Height);
                if (((LabelStyle)_style).Background != null)
                {
                    dcGs.DrawImage(((LabelStyle)_style).Background, new RectangleF(_rect.X + 1, _rect.Y + 1, _rect.Width - 1, _rect.Height - 1));
                }
                else if (((LabelStyle)_style).BackgroundBrush != null)
                {
                    dcGs.FillRectangle(((LabelStyle)_style).BackgroundBrush, new RectangleF(_rect.X + 1, _rect.Y + 1, _rect.Width - 1, _rect.Height - 1));
                }
            }
            else
            {
                if (((LabelStyle)_style).Background != null)
                {
                    dcGs.DrawImage(((LabelStyle)_style).Background, _rect);
                }
                else if (((LabelStyle)_style).BackgroundBrush != null)
                {
                    dcGs.FillRectangle(((LabelStyle)_style).BackgroundBrush, _rect);
                }
            }
