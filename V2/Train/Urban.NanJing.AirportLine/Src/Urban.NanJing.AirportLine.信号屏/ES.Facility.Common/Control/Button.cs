...
namespace ES.Facility.Common.Control
{
    ...
    {
        private Boolean m_SelfReplication;//按钮自复标识
        ...
        public void Paint(Graphics g)
        {
            ...
            {
                g.DrawImage(((ButtonStyle)m_Style).DisableImage, m_Rect);
                g.DrawString(
                    m_Text,
            ...
            {
                g.DrawImage(((ButtonStyle)m_Style).DownImage, m_Rect);
            }
            else//按钮复位，绘制常规图片背景
            {
                g.DrawImage(m_Style.Background, m_Rect);
            }
            g.DrawString(
                m_Text,
                ((ButtonStyle)m_Style).FontStyle.Font,
