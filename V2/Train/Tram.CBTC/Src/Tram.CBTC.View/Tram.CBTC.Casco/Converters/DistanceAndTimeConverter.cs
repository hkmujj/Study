...
{
    public class DistanceAndTimeConverter : IMultiValueConverter
    {
        ...
        public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            var distacne = (float)(values[0]);
            var datetime = (DateTime)values[1];


            return string.Format("{0} 米 {1} 分 {2} 秒", distacne.ToString().PadRight(3, ' '), datetime.Minute.ToString().PadLeft(2, ' '), datetime.Second.ToString().PadLeft(2, ' '));
        }
