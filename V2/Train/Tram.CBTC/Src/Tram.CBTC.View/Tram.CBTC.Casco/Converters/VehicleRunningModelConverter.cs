...
{
    public class VehicleRunningModelConverter : IMultiValueConverter
    {
        ...
        {
            if (values == null || values == DependencyProperty.UnsetValue || values.All(a => a == null || a == DependencyProperty.UnsetValue))
            {
                return DependencyProperty.UnsetValue;
            }

            var tmp = (VehicleRunningModel)values[0];
            if (tmp == VehicleRunningModel.VehicleOnlineEqualinterval)
            {
                return values[1] + "\r\n" + values[2];
            }
            return values[1];
