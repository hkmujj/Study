...
{
    [Export]
    public class SS3BViewModel : NotificationObject, IDataListener, ISS3BViewModel
    {
        ...
        private void MMIBlackChanged(CommunicationDataChangedArgs<bool> changedBools)
        {
            ...
            if (changedBools.NewValue.ContainsKey(index))
            {
                ...
                {
                    BottomButtonViewModel.ChangedViewContent(LiuZhouViewFullNames.MasterMainPage);
                }
        ...
        private void MainCircuitBoolChanged(CommunicationDataChangedArgs<bool> args)
        {
            ...
            index = IndexConfigure.Instance.IndexFacade.InBoolDictionary[InBoolKeys.从车司机钥匙];
            if (args.NewValue.ContainsKey(index))
            {
                MainCircuitPageViewModel.DriverKeyOther = BoolToDouble(args.NewValue[index]);
            }
        ...
        private string m_CurrentView = LiuZhouRegionNames.MainContent;
        ...
        void Button(CommunicationDataChangedArgs args)
        {
            ...
            {
                if (m_CurrentView == LiuZhouViewFullNames.ModelSelectPage)
                {
                    m_CurrentView = BottomButtonViewModel.ChangedMainContent(LiuZhouViewFullNames.Main);
                    BottomButtonViewModel.ChangedViewContent(LiuZhouViewFullNames.MasterMainPage);
                }
                else
                {
                    BottomButtonViewModel.ChangedViewContent(LiuZhouViewFullNames.MainNetPage);
                }
                return;
            }
            ...
            if (args.ChangedBools.NewValue.ContainsKey(index) && args.ChangedBools.NewValue[index])
            {
                BottomButtonViewModel.ChangedViewContent(LiuZhouViewFullNames.FaultView);
            }
            index = IndexConfigure.Instance.IndexFacade.InBoolDictionary[InBoolKeys.调车2按下状态MMI2键按下状态];
            if (args.ChangedBools.NewValue.ContainsKey(index) && args.ChangedBools.NewValue[index])
            {
                BottomButtonViewModel.ChangedViewContent(LiuZhouViewFullNames.AssistSysytemPage);
            }
            index = IndexConfigure.Instance.IndexFacade.InBoolDictionary[InBoolKeys.车位3按下状态MMI3键按下状态];
            if (args.ChangedBools.NewValue.ContainsKey(index) && args.ChangedBools.NewValue[index])
            {
                BottomButtonViewModel.ChangedViewContent(LiuZhouViewFullNames.StatusPage);
            }
            index = IndexConfigure.Instance.IndexFacade.InBoolDictionary[InBoolKeys.进路号4按下状态MMI4键按下状态];
            if (args.ChangedBools.NewValue.ContainsKey(index) && args.ChangedBools.NewValue[index])
            {
                BottomButtonViewModel.ChangedViewContent(LiuZhouViewFullNames.MainCircuitPage);
            }
            index = IndexConfigure.Instance.IndexFacade.InBoolDictionary[InBoolKeys.定标5按下状态MMI5键按下状态];
            if (args.ChangedBools.NewValue.ContainsKey(index) && args.ChangedBools.NewValue[index])
            {
                BottomButtonViewModel.ChangedViewContent(LiuZhouViewFullNames.AxleTemperaturePage);
            }
            index = IndexConfigure.Instance.IndexFacade.InBoolDictionary[InBoolKeys.查询按钮按下];
            if (args.ChangedBools.NewValue.ContainsKey(index) && args.ChangedBools.NewValue[index])
            {
                BottomButtonViewModel.ChangedViewContent(LiuZhouViewFullNames.Tax2Page);
            }
            index = IndexConfigure.Instance.IndexFacade.InBoolDictionary[InBoolKeys.返回按钮按下];
            if (args.ChangedBools.NewValue.ContainsKey(index) && args.ChangedBools.NewValue[index])
            {
                m_CurrentView = BottomButtonViewModel.ChangedMainContent(LiuZhouViewFullNames.ModelSelectPage);
            }
