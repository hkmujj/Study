...
{
    public class TestOfEmergencyBrake : BrakeTestControl
    {
        public TestOfEmergencyBrake(BrakeTestType brakeTestType, ControlBrakeTest srcObj) : base(brakeTestType, srcObj)
        {
            BrakeTestResult = new BrakeTestResult("紧急制动试验");
        }
        ...
        protected override void UpdateContents()
        {
            switch (m_Step)
            {
                case 0:
                    ...
                    if (!GetInBoolValue(InBoolKeys.Inb制动测试1369))
                    {
                        m_Step = 1;
                    }
