...
{
    internal class AirconditionView8 : AirconditionviewBase
    {
        public AirconditionView8(CRH3C380BBase baseClass)
            : base(baseClass)
        {
            ...
            ConstInfoCollection.Add(new Line(line15.EndPoint, new Point(line15.EndPoint.X + 293, line15.EndPoint.Y)) { Color = txtColor, Tag = 18, Visible = baseClass.DMITitle.MarshallMode });
            //获取第一条Line
            ...
            ConstInfoCollection[ConstInfoCollection.FindIndex(f => f is Line && (int)f.Tag == 8)].Visible = baseClass.DMITitle.MarshallMode;
            ConstInfoCollection[ConstInfoCollection.FindIndex(f => f is Line && (int)f.Tag == 9)].Visible = baseClass.DMITitle.MarshallMode;
            ConstInfoCollection[ConstInfoCollection.FindIndex(f => f is Line && (int)f.Tag == 10)].Visible = baseClass.DMITitle.MarshallMode;
            ConstInfoCollection[ConstInfoCollection.FindIndex(f => f is Line && (int)f.Tag == 11)].Visible = baseClass.DMITitle.MarshallMode;
            ...
            AirconditionViewItemCollection.Add(new SelectableAirconditionViewItemDecorator(new AirconditionViewItem(AirconditionLevelUnit.Unit1)
            {
            ...
            for (int i = 11; i <= 18; i++)
            {
                ...
                AirconditionViewItemCollection.Add(new SelectableAirconditionViewItemDecorator(new AirconditionViewItem((AirconditionLevelUnit)i)
                {
            ...
            baseClass.UpdateUiObject(CommunicationDataType.InBool, AirconditionViewItemCollection.Select(s => s.AirconditionViewItemDecorater.Tag).Cast<string[]>().SelectMany(s => s).Distinct());
        ...
        private void Graphics(Color txtColor, int x1, int y4)
        {
            ...
            for (int i = 0; i < 7; i++)
            {
                if (i != 3)
                {
                    ...
                    //画刻度线
                    ConstInfoCollection.Add(new Line(new Point(x1 - 30, y4 + i * 10), new Point(x1, y4 + i * 10)) { Color = Color.White, Tag = 12 + i });
                }
                else
                {
                    //画刻度线
                    ConstInfoCollection.Add(new Line(new Point(x1 - 45, y4 + i * 10), new Point(x1 + 290, y4 + i * 10)) { Color = Color.White, Tag = 12 + i });
                    //画字符串"标准"
                    ConstInfoCollection.Add(new GDIRectText
                    {
                        TextColor = txtColor,
