...
{
    internal class AirConditionBLViewBase : CommonInnerControlBase, IAirconditionView
    {
        ...
        protected AirConditionBLViewBase(CRH3C380BBase baseClass)
        {
            BaseClass = baseClass;
            ...
            var regionSize = new Size(207, 155);
            ...
            var tmp = GetDisPlayTest();
            var index = 0;
            foreach (var key in tmp.Keys)
            {
                var location = new Point(x + interval, y + interval*(index + 1) + itemSize.Height*index);
            ...
            //    ConstInfoCollection.Add(new AirconditionViewItem { OutLineRectangle = new Rectangle(location, itemSize), State = (AirConditionState)i });
        ...
        private static Dictionary<AirConditionState, string> GetDisPlayTest()
        {
            var tmp = new Dictionary<AirConditionState, string>
            {
                {AirConditionState.Close, EnumUtil.GetDescription(AirConditionState.Close).FirstOrDefault()},
                {AirConditionState.AutoOpen, EnumUtil.GetDescription(AirConditionState.AutoOpen).FirstOrDefault()},
                {AirConditionState.ManualOpen, EnumUtil.GetDescription(AirConditionState.ManualOpen).FirstOrDefault()},
                {
                    AirConditionState.SetTemperature,
                    EnumUtil.GetDescription(AirConditionState.SetTemperature).FirstOrDefault()
                },
                {
                    AirConditionState.EmergaceClose,
                    EnumUtil.GetDescription(AirConditionState.EmergaceClose).FirstOrDefault()
                }
            };
        ...
        private void DrawAiecondition(AirconditionViewItem item, string[] indexName)
        {
            ...
            {
                item.State = AirConditionState.AutoOpen;
            }
            else if (BaseClass.GetInBoolValue(indexName[1]))
            {
                item.State = AirConditionState.ManualOpen;
            }
