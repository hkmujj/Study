...
{
    internal class AirconditionSelectStrategy : SelectStrategyBase
    {
        public AirconditionSelectStrategy(IList<SelectableAirconditionViewItemDecorator> selectableCollection)
            : base(selectableCollection.Cast<ISelectable>().ToList().AsReadOnly())
        {
            if (GlobalParam.Instance.ProjectType == ProjectType.CRH380BL)
            {
                var lst = new List<ISelectable>(selectableCollection.Skip(1).Take(8));
                lst.AddRange(selectableCollection.Skip(17).Take(8));
            ...
            {
                if (selectableCollection.Count > 18)
                {
                    var lst = new List<ISelectable>(selectableCollection.Skip(2).Take(8));
                    lst.AddRange(selectableCollection.Skip(19).Take(8));
                    Selectables = new List<List<ISelectable>>
                    {
                        new List<ISelectable> {selectableCollection[0]},
                        new List<ISelectable> {selectableCollection[1], selectableCollection[18]},
                        lst
                    };
                }
                else
                {
                    Selectables = new List<List<ISelectable>>
                    {
                        ...
                        new List<ISelectable>(selectableCollection.Skip(2).Take(8))
                    };
                }
            }
