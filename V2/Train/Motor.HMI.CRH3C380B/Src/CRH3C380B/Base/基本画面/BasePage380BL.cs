...
{
    public class BasePage380BL : BasePageContent
    {
        public BasePage380BL(DMIBasePage srcObj)
            : base(srcObj)
        {
        ...
        private void OnRefreshAction(object o)
        {
            ...
            for (var index = 0; index < m_FValue.Length; index++)
            {
                ...
                {
                    m_FValue[index] =
                        SrcObj.FloatList[SrcObj.GetInFloatIndex(m_FValueAllArrange[SrcObj.DMITitle.MarshallMode ? 1 : 0][index])];
                }
                else
                {
        ...
        private void PaintState(Graphics g)
        {
            ...
            g.DrawString(SrcObj.DMITitle.TrainInSide ? "16-09   08-01" : "01-08   09-16", FontsItems.FontE12,
                SrcObj.DMITitle.NightMode ? SolidBrushsItems.BlackBrush : SolidBrushsItems.WhiteBrush, m_RectsList[20],
            ...
            g.DrawString("", FontsItems.FontC11,
                SrcObj.DMITitle.NightMode ? SolidBrushsItems.BlackBrush : SolidBrushsItems.WhiteBrush,
                m_RectsList[19], FontsItems.TheAlignment(FontRelated.靠左));
            g.DrawString(SrcObj.DMITitle.TrainInSide
                ? "    15            10             07             02          "
                : "    02            07             10             15    ", FontsItems.FontE12,
                SrcObj.DMITitle.NightMode ? SolidBrushsItems.BlackBrush : SolidBrushsItems.WhiteBrush,
                m_RectsList[70], FontsItems.TheAlignment(FontRelated.靠左));

            g.DrawString((SrcObj.DMITitle.TrainInSide
                ? "16    14      11    09     08    06      03     01"
        ...
        private void PaintGroundImage(Graphics g)
        {
            ...
            {
                g.DrawImage(MSImages.voltage_1_1, m_RectsList[0]);
                g.DrawImage(MSImages.powerCurrent_1, m_RectsList[1]);
                g.DrawImage(MSImages.牵引图_1_1, m_RectsList[2]);
            }
            else
            {
                g.DrawImage(MSImages.voltage_0_1, m_RectsList[0]);
                g.DrawImage(MSImages.powerCurrent_0, m_RectsList[1]);
                g.DrawImage(MSImages.牵引图_0_1, m_RectsList[2]);
            }
