...
{
    internal class CloseCouplerHatchesView2 : CloseCouplerHatchesView1
    {
        public CloseCouplerHatchesView2(CRH3C380BBase baseClass)
            : base(baseClass)
        {
            var point3 =
                ((TrainGroupView) ConstInfoCollection.Find(f => f is TrainGroupView && (int) f.Tag == 2))
                    .LeftEdgeLocation;
            var point4 =
                ((TrainGroupView) ConstInfoCollection.Find(f => f is TrainGroupView && (int) f.Tag == 2))
                    .RightEdgeLocation;
            ...
            CloseCouplerHatchesViewItemCollection.Add(
                new SelectableCloseCouplerHatchesViewItemDecorator(new CouplerHatchesViewItem(CouplerHatchesUnit.All)
                {
                    ...
                    Tag =
                        new[]
                        {
                            InBoolKeys.Inb动车组2前车钩状态第0位,
                            InBoolKeys.Inb动车组2前车钩状态第1位,
                            InBoolKeys.Inb动车组2前车钩状态第2位
                        },
            ...
            CloseCouplerHatchesViewItemCollection.Add(
                new SelectableCloseCouplerHatchesViewItemDecorator(new CouplerHatchesViewItem(CouplerHatchesUnit.All)
                {
                    ...
                    Tag =
                        new[]
                        {
                            InBoolKeys.Inb动车组2后车钩状态第0位,
                            InBoolKeys.Inb动车组2后车钩状态第1位,
                            InBoolKeys.Inb动车组2后车钩状态第2位
                        },
