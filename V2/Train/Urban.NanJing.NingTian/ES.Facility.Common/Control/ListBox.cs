...
namespace ES.Facility.Common.Control
{
    ...
    public class ListBox<T> : IControl, IDisposable
    {
        ...
        {
            g.FillRectangle(_backgroundBrush, _rect);
            ...
            else
            {
                ...
                //丛线
                float xpos = _rect.X;
                g.DrawLine(new Pen(_gridBrush, 2), new PointF(xpos, _rect.Y), new PointF(xpos, _rect.Y + _rect.Height));
                xpos -= 1;
                for (int i = 0; i < _headers.Length; i++)
                {
                    xpos += _headers[i].Width + 2;
        ...
        {
            int pages = _dataList.Count / _rowCount;//总页数
            ...
            for (int i = 0; i <= _currentDataList.Count - 1; i++)//显示当前列表数据
            {
                ...
                for (int j = _headers[0].IsCount ? 1 : 0; j < _headers.Length; j++)//显示每列数据
                {
                    ...
                    {
                        //根据属性名称以及所在对象反射获取数据
                        string data = _currentDataList[i].GetType().GetProperty(_headers[j].TProperty).GetValue(_currentDataList[i], null).ToString();
        ...
        {
            int pages = _dataList.Count / _rowCount;
