...
namespace Urban.NanJing.NingTian.DDU.通讯状态
{
    ...
    public class V4_Message_C4_A2 : baseClass
    {
        #region 私有变量
        ...
        public override bool init(ref int nErrorObjectIndex)
        {
            ...
            string[] tmpFile0 = File.ReadAllLines(
                Path.Combine(AppPaths.ConfigDirectory, "A2模块信息.txt"),
                Encoding.Default
                );
        ...
        private void paint_TcMode(Graphics g)
        {
            ...
            for (int i = 0; i < UIObj.InBoolList.Count; i++)
            {
                ...
                {
                    g.DrawLine(
                        new Pen(new SolidBrush(Color.FromArgb(170, 169, 152)), 3),
                        new PointF(axis1 - 79, 217 + 31 / 2 + ((_modesInfo[i].ID - 1) * 33)),
                        new PointF(axis1, 217 + 31 / 2 + ((_modesInfo[i].ID - 1) * 33))
                        );
                    g.DrawImage(
                        _resource_Images[_values[i]],
                        new RectangleF(axis1 - 24 - 62, 217 + ((_modesInfo[i].ID - 1) * 33), 62, 31)
                        );
                    g.DrawString(
                        _modesInfo[i].Name,
                ...
                else
                {
                    g.DrawLine(
                        new Pen(new SolidBrush(Color.FromArgb(170, 169, 152)), 3),
                        new PointF(axis1, 217 + 31 / 2 + ((_modesInfo[i].ID - 1 - 7) * 33)),
                        new PointF(axis1 + 79, 217 + 31 / 2 + ((_modesInfo[i].ID - 1 - 7) * 33))
                        );
                    g.DrawImage(
                        _resource_Images[_values[i]],
                        new RectangleF(axis1 + 24, 217 + ((_modesInfo[i].ID - 1 - 7) * 33), 62, 31)
                        );
                    g.DrawString(
                        _modesInfo[i].Name,
                        new Font("宋体", 11f, FontStyle.Bold),
