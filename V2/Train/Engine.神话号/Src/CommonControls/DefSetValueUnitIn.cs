...
{
    public partial class DefSetValueUnitIn : PictureBox, ISetValue
    {
        ...
        public String DefUnit
        {
            ...
            set
            {
                if (_defUnit == value) return;
                _defUnit = value;

                _defLabel_Unit_1.DefText = value;
                _defLabel_Unit_2.DefText = value;
            }
        }
        private String _defUnit = "单位";
        ...
        public DefSetValueUnitIn()
        {
            InitializeComponent();

            _defLabels_SetValue = new List<DefLabel>()
            {
                _defLabel_Set_Wight_1,_defLabel_Set_Wight_2,_defLabel_Set_Wight_3,_defLabel_Set_Wight_4,_defLabel_Set_Wight_5
            };
            _defLabels_CurValue = new List<DefLabel>()
            {
                _defLabel_Cur_Wight_1,_defLabel_Cur_Wight_2,_defLabel_Cur_Wight_3,_defLabel_Cur_Wight_4,_defLabel_Cur_Wight_5
            };
        }
        ...
        public Int32 GetValue()
        {
            ...
            if (strValue == "")
            {
                for (int i = 0; i < _defLabels_CurValue.Count; i++)
                {
                    if (_defLabels_CurValue[i].DefText != "")
                    {
                        strValue += _defLabels_CurValue[i].DefText;
                    }
                    else strValue += "0";
                }
            }
