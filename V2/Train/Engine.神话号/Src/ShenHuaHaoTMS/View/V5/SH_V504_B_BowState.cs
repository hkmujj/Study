...
{
    public partial class SH_V504_B_BowState : UserControl, IView, IDisposable, IDataListener
    {
        ...
        private List<TableStateInfo> _tableStateInfos = new List<TableStateInfo>();
        ...
        public SH_V504_B_BowState()
        {
            InitializeComponent();
        }

        public SH_V504_B_BowState(Int32 id, ViewManager viewManager, ICommunicationDataService dataService, BlackView bv,
        ...
        public void SetData(DataSet ds)
        {
            ...
            DataRowCollection drc = ds.Tables["B车-受电弓"].Rows;
        ...
        public void OnBoolChanged(object sender, CommunicationDataChangedArgs<bool> e)
        {
            foreach (var cmd in e.NewValue)
            {
                ...
                if (cmd.Key == 1323)
                {
                    if (cmd.Value) //编组成功
                    {
                        _panel_ViewBtns.InvokeRemoveChild(_btn_Null_3);
