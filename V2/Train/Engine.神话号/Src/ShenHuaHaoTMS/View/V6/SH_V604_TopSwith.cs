...
{
    public partial class SH_V604_TopSwith : UserControl, IView, IDisposable, IDataListener
    {
        ...
        private List<DefState> _states = new List<DefState>();
        private List<ILogic> _statesControls = new List<ILogic>();

        public SH_V604_TopSwith()
        {
            InitializeComponent();
        }

        public SH_V604_TopSwith(Int32 id, ViewManager viewManager, ICommunicationDataService dataService, BlackView bv,
        ...
        public void OnBoolChanged(object sender, CommunicationDataChangedArgs<bool> e)
        {
            foreach (var cmd in e.NewValue)
            {
                ...
                if (cmd.Value)
                {
                    if (cmd.Key == 869)
                    {
                        _btn_3.DefText = "RDC1_OFF";
                    }
                    else if (cmd.Key == 870)
                    {
                        _btn_3.DefText = "RDC1_ON";
                    }
                }
            }
        }
        ...
        private void DefBtnClick(object sender, ButtonClickArgs e)
        {
