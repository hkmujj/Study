...
namespace Subway.ShiJiaZhuangLine1.Subsystem.Model
{
    public class MainModel : NotificationObject
    {
        ...
        private bool m_TrainRightDoorOneOpen;
        private bool m_TrainRightDoorSixOpen;
        private bool m_TrainRunLeft;
        private bool m_TrainLeftDoorOneOpen;
        private bool m_TrainLeftDoorSixOpen;
        ...
        /// 1号车左侧门开启
        /// </summary>
        public bool TrainLeftDoorOneOpen
        {
            get { return m_TrainLeftDoorOneOpen; }
            set
            {
                if (value == m_TrainLeftDoorOneOpen)
                {
                    return;
                }
                m_TrainLeftDoorOneOpen = value;
                RaisePropertyChanged(() => TrainLeftDoorOneOpen);
            }
        }
        ...
        /// 6号车左侧门开启
        /// </summary>
        public bool TrainLeftDoorSixOpen
        {
            get { return m_TrainLeftDoorSixOpen; }
            set
            {
                if (value == m_TrainLeftDoorSixOpen)
                {
                    return;
                }
                m_TrainLeftDoorSixOpen = value;
                RaisePropertyChanged(() => TrainLeftDoorSixOpen);
            }
        }
        /// <summary>
        /// 左侧运行方向
        ...
        /// 1号车右侧车门开启
        /// </summary>
        public bool TrainRightDoorOneOpen
        {
            get { return m_TrainRightDoorOneOpen; }
            set
            {
                if (value == m_TrainRightDoorOneOpen)
                {
                    return;
                }
                m_TrainRightDoorOneOpen = value;
                RaisePropertyChanged(() => TrainRightDoorOneOpen);
            }
        }
        /// <summary>
        /// 6号车右侧车门开启
        /// </summary>
        public bool TrainRightDoorSixOpen
        {
            get { return m_TrainRightDoorSixOpen; }
            set
            {
                if (value == m_TrainRightDoorSixOpen)
                {
                    return;
                }
                m_TrainRightDoorSixOpen = value;
                RaisePropertyChanged(() => TrainRightDoorSixOpen);
            }
        }
        /// <summary>
        /// 运行方向右侧
        ...
        public void ChangeBools(CommunicationDataChangedArgs<bool> args)
        {
            ...
            args.UpdateIfContains(InBoolKeys.Inb1号车列车左侧预开门, b => TrainLeftDoorOneOpen = b);
            args.UpdateIfContains(InBoolKeys.Inb1号车列车右侧预开门, b => TrainRightDoorOneOpen = b);
            args.UpdateIfContains(InBoolKeys.Inb6号车列车左侧预开门, b => TrainLeftDoorSixOpen = b);
            args.UpdateIfContains(InBoolKeys.Inb6号车列车右侧预开门, b => TrainRightDoorSixOpen = b);
            ...
            args.UpdateIfContains(InBoolKeys.Inb车1号通道门紧急解锁激活_闪烁_, b =>
            {
                if (b) { IntervalOneDoorState = IntervalDoorState.UnlockOrOpen; }
            });
            args.UpdateIfContains(InBoolKeys.Inb车1号通道门紧急解锁未激活, b =>
                {
                    if (b)
                    {
            ...
            args.UpdateIfContains(InBoolKeys.Inb1号车通道门状态未知, b => { if (b) { IntervalOneDoorState = IntervalDoorState.UnKnow; } });

            args.UpdateIfContains(InBoolKeys.Inb车6号通道门紧急解锁激活_闪烁_, b =>
            {
                if (b)
                {
            ...
            args.UpdateIfContains(InBoolKeys.Inb车6号通道门紧急解锁未激活, b =>
            {
                if (b)
                {
            ...
            args.UpdateIfContains(InBoolKeys.Inb6号车通道门状态未知, b => { if (b) { IntervalSixDoorState = IntervalDoorState.UnKnow; } });
        }
