    ...
    public partial class Title : baseClass
    {
        ...
        private void DrawFrameUp(Graphics e)
        {
            ...
            if (MsgInf.CurrentMsgList.Count != 0)
            {
                ...
                e.FillRectangle(FormatStyle.RedBrush, rects[52]);
                e.DrawString(MsgInf.CurrentMsgList[0].MsgContent, FormatStyle.Font12B, FormatStyle.WhiteBrush, rects[52], LeftFormat);
            }


            //时间
            e.DrawString(this.CurrentTime().ToString("yyyy-MM-dd"), FormatStyle.Font14B,
                FormatStyle.WhiteBrush, rects[6], drawFormat);
            e.DrawString(this.CurrentTime().ToLongTimeString(), FormatStyle.Font14B,
                FormatStyle.WhiteBrush, rects[7], drawFormat);

            //下一站
            e.DrawString("当前站", FormatStyle.Font14B, FormatStyle.WhiteBrush, rects[8], drawFormat);
            if (TrainStation(Convert.ToInt32(fValue[1]), nextStation) == null)
                ...
                e.DrawString(TrainStation(Convert.ToInt32(fValue[1]), nextStation), FormatStyle.Font12B,
            ...
            if (TrainStation(Convert.ToInt32(fValue[2]), nextStation) == null)
                ...
                e.DrawString(TrainStation(Convert.ToInt32(fValue[2]), nextStation), FormatStyle.Font12B,
        ...
        private void DrawFrameDown(Graphics e)
        {
            ...
            for (int i = 0; i < 7; i++)
            {
                ...
                if (i != 5)
                    e.DrawString(FormatStyle.Str1[i], FormatStyle.Font12B, FormatStyle.BlackBrush, rects[21 + i], drawFormat);
                else
        ...
        private void DrawOn(Graphics e)
        {
            ...
            if (isShowCar)
            {
                ...
                for (int i = 0; i < 6; i++)
                {
                    e.DrawString(维护.TrainNumSetup.TrainID[i], FormatStyle.Font12B,
            ...
            if (isShowDirection)
            {
                ...
                else if (BoolList[203])
                    e.DrawImage(Img[13], rects[51]);
                else if (BoolList[198])
                    e.DrawImage(Img[13], rects[50]);
                else if (BoolList[199])
                    e.DrawImage(Img[12], rects[51]);
            }
        }
