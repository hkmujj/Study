    ...
    public partial class MMI_Main : baseClass
    {
        ...
        private void DrawAreaB(Graphics e)
        {
            if (bValue[18])
            {
                ...
                {
                    dialTX.OnDraw(e, FormatStyle.Font20);
                    pointer = Img[0];
                    speedPointerTX.PaintPointer(e, pointer, ref fValue[2]);
                }
            }
            else
            {
                ...
                if (!BoolList[UIObj.InBoolList[50]])
                {
        ...
        private void DrawAreaE(Graphics e)
        {
            ...
            if (MMI_MsgReceive.msgInf.NeedShowHighMsg)
            {
                if (MMI_MsgReceive.msgInf.MsgHasShowed(50) == 1)
                {
                    ...
                    //消息
                    e.DrawString(MMI_MsgReceive.msgInf.HighMsg.MsgContent,
            ...
            else
            {
                ...
                {
                    var count = MMI_MsgReceive.msgInf.AllMsgsList.Count;
                    var index = count - RowId;
                    ...
                    if (count > 0 && count <= 3)
                    {
                        ...
                        {
                            e.DrawString(MMI_MsgReceive.msgInf.AllMsgsList[count - i - 1].MsgReceiveTime.ToShortTimeString() +
                    ...
                    {
                        for (var i = 0; i < (index > 3 ? 3 : index); i++)
                        {
                            e.DrawString(MMI_MsgReceive.msgInf.AllMsgsList[count - i - 1 - RowId].MsgReceiveTime.ToShortTimeString() +
                                "  " + MMI_MsgReceive.msgInf.AllMsgsList[count - i - 1 - RowId].MsgContent, FormatStyle.Font12B,
                                FormatStyle.WhiteSolidBrush, rects[62 + i], LeftFormat);
                        }
                    }
                }
        ...
        {
            var date = DateTime.Today.ToString("MM") + "/" + DateTime.Today.ToString("dd")
                        + "/" + DateTime.Today.ToString("yyyy");
            var time = this.NowTime().ToLongTimeString();
            //G1
            //生命显示
            LifeIndicator();
            if (!BoolList[UIObj.InBoolList[51]])
            ...
            switch (liftIndic)
            {
                ...
                case 7:
                    ...
                    e.FillRectangle(FormatStyle.DarkGreySolidBrush, rects[47]);
                    e.FillRectangle(FormatStyle.LightGreySolidBrush, rects[100]);
                    break;
            }

            //G2
            //当前日期
            e.DrawString(date, FormatStyle.Font16B,
                FormatStyle.WhiteSolidBrush, rects[54], drawFormat);
            //G3
            //当前时间
            e.DrawString(time, FormatStyle.Font16B,
        ...
        private void DrawAreaM(Graphics e)
        {
            ...
            if (bValue[42])
                ...
                e.DrawImage(Img[54], rects[26]);
            else if (bValue[47])
                e.DrawImage(Img[53], rects[26]);
        }
        ...
        {
            var stopTime = Convert.ToInt32(fValue[9]);
        ...
        private void DrawACK(Graphics e)
        {
            ...
            e.DrawString("后备模式选择", FormatStyle.Font14B, FormatStyle.WhiteSolidBrush,
                rects[97], drawFormat);
            for (var i = 0; i < 2; i++)
            {
