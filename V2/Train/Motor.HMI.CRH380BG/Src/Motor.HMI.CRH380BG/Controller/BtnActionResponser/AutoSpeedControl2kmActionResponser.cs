...
{
    [Export]
    class AutoSpeedControl2kmActionResponser : BtnActionResponserBase
    {
         /// <summary>
         /// 响应按键
         /// </summary>
        ...
        {
            EventAggregator.GetEvent<SendDataRequestEvent>().Publish(new SendDataRequestEvent.Args(OutBoolKeys.Oub自动速度控制_2kmh连挂速度, true));

            EventAggregator.GetEvent<SendDataRequestEvent>().Publish(new SendDataRequestEvent.Args(OutBoolKeys.Oub自动速度控制_5kmh, false));
            ...
            stateViewModel.DomainViewModel.Domain.Model.MainData.AutoSpeedControlType = AutoSpeedControlType.Speed2;
        }
    }
}
