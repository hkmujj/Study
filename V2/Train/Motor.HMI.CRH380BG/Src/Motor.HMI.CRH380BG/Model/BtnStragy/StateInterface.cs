...
namespace Motor.HMI.CRH380BG.Model.BtnStragy
{
    ...
    public class StateInterface : NotificationObject, IStateInterface
    {
        ...
        private readonly IRegionManagerProvider m_RegionManagerProvider;
        ...
        private BtnItem m_BtnBSwitch;
        private BtnItem m_BtnBLight;
        private BtnItem m_BtnBDayandnightSwitch;
        private BtnItem m_BtnBLanguageSelect;
        private BtnItem m_BtnBIofoBox;
        private BtnItem m_BtnBFaultInfo;
        private BtnItem m_BtnBTrainRunningCheck;
        private BtnItem m_BtnBTrainStopCheck;
        private BtnItem m_BtnBSwitchDisplay;
        private BtnItem m_BtnBReturn;
        private BtnItem m_BtnBOK;

        public StateInterface()
        {
            m_RegionManagerProvider = ServiceLocator.Current.GetInstance<IRegionManagerProvider>();
        }
        ...
        public BtnItem BtnBSwitchDisplay
        {
            set
            {
                if (Equals(value, m_BtnBSwitchDisplay))
                {
                    return;
                }

                m_BtnBSwitchDisplay = value;
                RaisePropertyChanged(() => BtnBSwitchDisplay);
            }
            get { return m_BtnBSwitchDisplay; }
        }

        public BtnItem BtnBTrainStopCheck
        {
            set
            {
                if (Equals(value, m_BtnBTrainStopCheck))
                {
                    return;
                }

                m_BtnBTrainStopCheck = value;
                RaisePropertyChanged(() => BtnBTrainStopCheck);
            }
            get { return m_BtnBTrainStopCheck; }
        }

        public BtnItem BtnBTrainRunningCheck
        {
            set
            {
                if (Equals(value, m_BtnBTrainRunningCheck))
                {
                    return;
                }

                m_BtnBTrainRunningCheck = value;
                RaisePropertyChanged(() => BtnBTrainRunningCheck);
            }
            get { return m_BtnBTrainRunningCheck; }
        }

        public BtnItem BtnBFaultInfo
        {
            set
            {
                if (Equals(value, m_BtnBFaultInfo))
                {
                    return;
                }

                m_BtnBFaultInfo = value;
                RaisePropertyChanged(() => BtnBFaultInfo);
            }
            get { return m_BtnBFaultInfo; }
        }

        public BtnItem BtnBIofoBox
        {
            set
            {
                if (Equals(value, m_BtnBIofoBox))
                {
                    return;
                }

                m_BtnBIofoBox = value;
                RaisePropertyChanged(() => BtnBIofoBox);
            }
            get { return m_BtnBIofoBox; }
        }

        public BtnItem BtnBLanguageSelect
        {
            set
            {
                if (Equals(value, m_BtnBLanguageSelect))
                {
                    return;
                }

                m_BtnBLanguageSelect = value;
                RaisePropertyChanged(() => BtnBLanguageSelect);
            }
            get { return m_BtnBLanguageSelect; }
        }

        public BtnItem BtnBDayandnightSwitch
        {
            set
            {
                if (Equals(value, m_BtnBDayandnightSwitch))
                {
                    return;
                }

                m_BtnBDayandnightSwitch = value;
                RaisePropertyChanged(() => BtnBDayandnightSwitch);
            }
            get { return m_BtnBDayandnightSwitch; }
        }

        public BtnItem BtnBSwitch
        {
            set
            {
                if (Equals(value, m_BtnBSwitch))
                {
                    return;
                }

                m_BtnBSwitch = value;
                RaisePropertyChanged(() => BtnBSwitch);
            }
            get { return m_BtnBSwitch; }
        }

        public BtnItem BtnBLight
        {
            set
            {
                if (Equals(value, m_BtnBLight))
                {
                    return;
                }

                m_BtnBLight = value;
                RaisePropertyChanged(() => BtnBLight);
            }
            get { return m_BtnBLight; }
        }
        ...
        private IRegionManager GetRegionManager(StateViewModel stateViewModel)
        {
            switch (stateViewModel.Model.ViewLocation)
            {
                case ViewLocation.Main:
                    return m_RegionManagerProvider.MainRegionManager;
                case ViewLocation.Reserve:
                    return m_RegionManagerProvider.ReserveRegionManager;
                default:
                    throw new ArgumentOutOfRangeException();
            }
        }

        public void UpdateState(StateViewModel stateViewModel)
        {
            try
            {
                GetRegionManager(stateViewModel).RequestNavigate(RegionNames.ContentContent, ContentViewName);
            }
            catch (Exception e)
            {
            ...
            BtnBSwitchDisplay.UpdateState();
            BtnBTrainStopCheck.UpdateState();
            BtnBTrainRunningCheck.UpdateState();
            BtnBFaultInfo.UpdateState();
            BtnBIofoBox.UpdateState();
            BtnBLanguageSelect.UpdateState();
            BtnBDayandnightSwitch.UpdateState();
            BtnBSwitch.UpdateState();
            BtnBLight.UpdateState();
        }
