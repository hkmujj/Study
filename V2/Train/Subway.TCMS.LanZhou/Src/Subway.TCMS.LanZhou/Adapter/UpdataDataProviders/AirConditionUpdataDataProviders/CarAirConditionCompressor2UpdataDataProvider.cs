...
namespace Subway.TCMS.LanZhou.Adapter.UpdataDataProviders.AirConditionUpdataDataProviders
{
    ...
    public class CarAirConditionCompressor2UpdataDataProvider : UpdateDataProviderBase
    {
        [ImportingConstructor]
        public CarAirConditionCompressor2UpdataDataProvider(IEventAggregator eventAggregator, DomainViewModel viewModel)
        ...
        {
            var d = ViewModel.TrainViewModel.Model.CarModels.Select(s => s.AirConditionControl.AirConditionStatusLazy.Value.AirConditionCompressor2Status).Where(w => !string.IsNullOrWhiteSpace(w.ItemConfig.CompressorStop2));
        ...
        {
            if (DataService.ReadService.GetInBoolOf(it.ItemConfig.CompressorRunning2))
            {
