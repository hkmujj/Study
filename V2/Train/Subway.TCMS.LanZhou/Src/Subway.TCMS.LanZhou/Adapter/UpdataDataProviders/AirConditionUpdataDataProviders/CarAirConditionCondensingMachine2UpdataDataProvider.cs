...
namespace Subway.TCMS.LanZhou.Adapter.UpdataDataProviders.AirConditionUpdataDataProviders
{
    ...
    class CarAirConditionCondensingMachine2UpdataDataProvider : UpdateDataProviderBase
    {
        [ImportingConstructor]
        public CarAirConditionCondensingMachine2UpdataDataProvider(IEventAggregator eventAggregator, DomainViewModel viewModel)
        ...
        {
            var d = ViewModel.TrainViewModel.Model.CarModels.Select(s => s.AirConditionControl.AirConditionStatusLazy.Value.AirConditionCondensing2Status).Where(w => !string.IsNullOrWhiteSpace(w.ItemConfig.CondensingStop2));       
        ...
        {
            if (DataService.ReadService.GetInBoolOf(it.ItemConfig.CondensingRunning2))
            {
