...
namespace Subway.TCMS.LanZhou.Adapter.UpdataDataProviders.AirConditionUpdataDataProviders
{
    ...
    class CarAirConditionVentilator4UpdataDataProvider : UpdateDataProviderBase
    {
        [ImportingConstructor]
        public CarAirConditionVentilator4UpdataDataProvider(IEventAggregator eventAggregator, DomainViewModel viewModel)
        ...
        {
            var d = ViewModel.TrainViewModel.Model.CarModels.Select(s => s.AirConditionControl.AirConditionStatusLazy.Value.AirConditionVentilator4Status).Where(w => !string.IsNullOrWhiteSpace(w.ItemConfig.VentilatorStop4));
        ...
        {
            if (DataService.ReadService.GetInBoolOf(it.ItemConfig.VentilatorRunning4))
            {
