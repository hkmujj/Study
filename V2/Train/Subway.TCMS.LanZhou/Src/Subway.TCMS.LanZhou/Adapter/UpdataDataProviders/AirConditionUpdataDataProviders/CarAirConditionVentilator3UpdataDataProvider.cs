...
namespace Subway.TCMS.LanZhou.Adapter.UpdataDataProviders.AirConditionUpdataDataProviders
{
    ...
    class CarAirConditionVentilator3UpdataDataProvider : UpdateDataProviderBase
    {
        [ImportingConstructor]
        public CarAirConditionVentilator3UpdataDataProvider(IEventAggregator eventAggregator, DomainViewModel viewModel)
        ...
        {
            var d = ViewModel.TrainViewModel.Model.CarModels.Select(s => s.AirConditionControl.AirConditionStatusLazy.Value.AirConditionVentilator3Status).Where(w => !string.IsNullOrWhiteSpace(w.ItemConfig.VentilatorStop3));
        ...
        {
            if (DataService.ReadService.GetInBoolOf(it.ItemConfig.VentilatorRunning3))
            {
