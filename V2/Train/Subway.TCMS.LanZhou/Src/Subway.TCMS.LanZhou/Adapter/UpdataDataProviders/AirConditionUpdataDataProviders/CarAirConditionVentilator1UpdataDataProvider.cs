...
namespace Subway.TCMS.LanZhou.Adapter.UpdataDataProviders.AirConditionUpdataDataProviders
{
    ...
    class CarAirConditionVentilator1UpdataDataProvider : UpdateDataProviderBase
    {
        [ImportingConstructor]
        public CarAirConditionVentilator1UpdataDataProvider(IEventAggregator eventAggregator, DomainViewModel viewModel)
        ...
        {
            var d = ViewModel.TrainViewModel.Model.CarModels.Select(s => s.AirConditionControl.AirConditionStatusLazy.Value.AirConditionVentilator1Status).Where(w => !string.IsNullOrWhiteSpace(w.ItemConfig.VentilatorStop1));
        ...
        {
            if (DataService.ReadService.GetInBoolOf(it.ItemConfig.VentilatorRunning1))
            {
