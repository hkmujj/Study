...
namespace Subway.TCMS.LanZhou.Adapter.UpdataDataProviders.AirConditionUpdataDataProviders
{
    ...
    public class CarAirConditionCompressor3UpdataDataProvider : UpdateDataProviderBase
    {
        [ImportingConstructor]
        public CarAirConditionCompressor3UpdataDataProvider(IEventAggregator eventAggregator, DomainViewModel viewModel)
        ...
        {
            var d = ViewModel.TrainViewModel.Model.CarModels.Select(s => s.AirConditionControl.AirConditionStatusLazy.Value.AirConditionCompressor3Status).Where(w => !string.IsNullOrWhiteSpace(w.ItemConfig.CompressorStop3));
        ...
        {
            if (DataService.ReadService.GetInBoolOf(it.ItemConfig.CompressorRunning3))
            {
