...
namespace Subway.TCMS.LanZhou.Adapter.UpdataDataProviders.TrainStatusUpdataDataProviders
{
    ...
    class CarBogieResectionStatus1UpdataDataProvider : UpdateDataProviderBase
    {
        [ImportingConstructor]
        public CarBogieResectionStatus1UpdataDataProvider(IEventAggregator eventAggregator, DomainViewModel viewModel)
        ...
        {
            var d = ViewModel.TrainViewModel.Model.CarModels.Select(s => s.TrainStatusDatas.TrainStatusBrakeViewLazy.Value.BogieResectionStatus1).Where(w => !string.IsNullOrWhiteSpace(w.ItemConfig.Bogie1ResectionNormal));
            ;
        ...
        {
            if (DataService.ReadService.GetInBoolOf(it.ItemConfig.Bogie1Resection))
            {
