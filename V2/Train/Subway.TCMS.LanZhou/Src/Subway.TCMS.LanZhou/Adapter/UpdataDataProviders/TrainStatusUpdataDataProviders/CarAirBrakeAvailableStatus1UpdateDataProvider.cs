...
namespace Subway.TCMS.LanZhou.Adapter.UpdataDataProviders.TrainStatusUpdataDataProviders
{
    ...
    class CarAirBrakeAvailableStatus1UpdateDataProvider : UpdateDataProviderBase
    {
        [ImportingConstructor]
        public CarAirBrakeAvailableStatus1UpdateDataProvider(IEventAggregator eventAggregator, DomainViewModel viewModel)
        ...
        {
            var d = ViewModel.TrainViewModel.Model.CarModels.Select(s => s.TrainStatusDatas.TrainStatusBrakeViewLazy.Value.AirBrakeAvailableStatus1).Where(w => !string.IsNullOrWhiteSpace(w.ItemConfig.AirBrake1AvailableNotavailable));
            ;
        ...
        {
            if (DataService.ReadService.GetInBoolOf(it.ItemConfig.AirBrake1AvailableNormal))
            {
