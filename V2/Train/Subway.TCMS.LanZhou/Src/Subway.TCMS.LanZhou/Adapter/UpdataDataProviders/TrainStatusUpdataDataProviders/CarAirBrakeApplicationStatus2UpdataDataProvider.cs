...
namespace Subway.TCMS.LanZhou.Adapter.UpdataDataProviders.TrainStatusUpdataDataProviders
{
    ...
    class CarAirBrakeApplicationStatus2UpdataDataProvider : UpdateDataProviderBase
    {
        [ImportingConstructor]
        public CarAirBrakeApplicationStatus2UpdataDataProvider(IEventAggregator eventAggregator, DomainViewModel viewModel)
        ...
        {
            var d = ViewModel.TrainViewModel.Model.CarModels.Select(s => s.TrainStatusDatas.TrainStatusBrakeViewLazy.Value.AirBrakeApplicationStatus2).Where(w => !string.IsNullOrWhiteSpace(w.ItemConfig.AirBrake2ApplicationExert));
        ...
        {
            if (DataService.ReadService.GetInBoolOf(it.ItemConfig.AirBrake2ApplicationNotapplied))
            {
