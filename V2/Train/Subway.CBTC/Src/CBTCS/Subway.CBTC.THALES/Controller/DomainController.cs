...
namespace Subway.CBTC.THALES.Controller
{
    ...
    public class DomainController : ControllerBase<Lazy<DomainViewModel>>
    {
        ...
        public ICommand InfoConfirm { get; private set; }
        public ICommand FaultConfirm { get; private set; }
            ...
            IRegionManager regionManager, IStateInterfaceFactory stateInterfaceFactory, NavigatorController navigator, RunningController runningController) : base(viewModel)
        {
            ...
            InfoConfirm = new DelegateCommand(() =>
            {
                var sendInfo = ViewModel.Value.CBTC.Message.FirstInfoShowItem;
                if (sendInfo == null)
                {
                    return;
                }
                SendInterface.EnsureMessage(
                    new SendModel<IInformationContent>(sendInfo.InformationContent));
            });
            FaultConfirm = new DelegateCommand(() =>
            {
                var sendInfo = ViewModel.Value.CBTC.Message.FirstFaultShowItem;
                if (sendInfo == null)
                {
                    return;
                }
