...
{
    public class EmergencyInfoConverter : IMultiValueConverter
    {
        ...
        public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            var value1 = (DoorState)values[0];
            var value2 = (DoorState)values[1];
            var value3 = (RealTimeWokeStatus)values[2];
            var value4 = (BrakeState)values[3];

            if (value3 == RealTimeWokeStatus.EB)
            {
                return values[6];
            }

            if (value1 == DoorState.Unkown || value2 == DoorState.Unkown)
            {
                return values[4];
            }

            if (value1 == DoorState.Opend || value2 == DoorState.Opend)
            {
                return values[5];
            }

            if (value4 == BrakeState.NormalRacing)
            {
                return values[7];
            }
