...
{
    public class StationParkingConverter : IMultiValueConverter
    {
        ...
        public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            var value1 = (ATCRequestState)values[0];
            var value2 = (JumpStopDetainCar)values[1];
            if (value1 == ATCRequestState.RequestClose)
            {
                return values[2];
            }

            if (value1 == ATCRequestState.ATCRequestLeave)
            {
                return values[3];
            }

            if (value2 == JumpStopDetainCar.DetainCar)
            {
                return values[4];
            }
            return null;
