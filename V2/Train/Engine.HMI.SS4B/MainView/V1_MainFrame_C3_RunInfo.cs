...
namespace SS4B_TMS.MainView
{
    ...
    public class V1MainFrameC3RunInfo : baseClass
    {
        /// <summary>
        ///     界面绘制
        /// </summary>
        ...
        public override void paint(Graphics dcGs)
        {
            ...
            dcGs.FillRectangle(m_LabelFillBrush, m_FillFrameRect);
            ...
            //TODO This Logic Error of Logic 3
            if (BoolList[UIObj.InBoolList[1]])
            {
                dcGs.DrawString(m_ChangedStrs[4], m_ChineseSmallFont, Brushs.YellowBrush, m_ChangedRects[4], FontInfo.SfCc);
            }
            else
            {
                dcGs.DrawString(m_ChangedDefaultStrs[4], m_ChineseSmallFont, Brushs.WhiteBrush, m_ChangedRects[4],
                    FontInfo.SfCc);
            }

            m_ChangedStrs[5] = GetInFloatValue(InFloatKeys.InFA轴位温度).ToString();
            dcGs.DrawString(m_ChangedStrs[5], m_ChineseSmallFont, Brushs.WhiteBrush, m_ChangedRects[5], FontInfo.SfCc);
        ...
        private readonly Rectangle m_FrameRect = new Rectangle(509, 1, 291, 538);

        private readonly Pen m_FramePen = new Pen(new SolidBrush(Color.FromArgb(128, 128, 128)), 4);

        private readonly SolidBrush m_LabelFillBrush = new SolidBrush(Color.FromArgb(9, 38, 190));

        private readonly Rectangle m_FillFrameRect = new Rectangle(510, 154, 289, 384);
        ...
        {
            m_FixedRects = new Rectangle[6];
            m_ChangedRects = new Rectangle[6];
            m_FillfixedRects = new Rectangle[6];
            for (m_I = 0; m_I < 6; m_I++)
            {
                m_FixedRects[m_I] = new Rectangle(509, 1 + m_I * 26, 124, 26);
                m_FillfixedRects[m_I] = new Rectangle(510, 2 + m_I * 26, 123, 25);
                m_ChangedRects[m_I] = new Rectangle(633, 1 + m_I * 26, 166, 26);
            }

            m_FixedStrs = new string[6] { "日期时间", "本次运行里程", "本次运行时间", "加馈标志", "LCU状态", "轴温" };
            ...
            m_ChangedDefaultStrs = new string[6] { "", "", "", "未加馈", "", "" };
