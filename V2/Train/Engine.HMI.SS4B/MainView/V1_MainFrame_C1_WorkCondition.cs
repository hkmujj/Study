...
namespace SS4B_TMS.MainView
{
    ...
    public class V1MainFrameC1WorkCondition : baseClass
    {
        public readonly string[] m_LogicName1 = new string[]
        {
        ...
        public readonly string[] m_LogicName2 = new string[]
       {
            InBoolKeys.InB本车机车工况蓄电池合,
            InBoolKeys.InB本车机车工况主断合,
            InBoolKeys.InB本车机车工况牵引,
            InBoolKeys.InB本车机车工况制动,
       };
        ...
        public override void paint(Graphics dcGs)
        {
            ...
            for (m_I = 0; m_I < 6; m_I++)
            {
                dcGs.DrawString(m_FixedRectStrs[2 * m_I], m_ChineseFont, Brushs.BlackBrush, m_FixedRects[2 * m_I],
                    FontInfo.SfCb);
            ...
            for (m_I = 0; m_I < 6; m_I++)
            {
                if (GetInBoolValue(m_LogicName1[m_I]))
                {
                ...
                {
                    dcGs.DrawString(m_ChangeableDefaultStrs[m_I], m_ChineseFont, Brushs.GreenBrush,
                        m_ChangeableRects[m_I],
                        FontInfo.SfCb);
                }
            }
            ...
            for (m_I = 0; m_I < 4; m_I++)
            {
                if (GetInBoolValue(m_LogicName2[m_I]))
                {
            ...
            {
                dcGs.DrawString(m_WorkConditionStrs[4], m_BigFont, Brushs.GreenBrush, m_ChangeableRects[6],
                    FontInfo.SfCc);
            }
            ...
            dcGs.DrawString(m_ChangeableRectStrs[7], m_BigDigitFont, Brushs.GreenBrush, m_ChangeableRects[7],
                FontInfo.SfRc);

            m_ChangeableRectStrs[8] = GetInFloatValue(InFloatKeys.InF机车速度).ToString("0");
            dcGs.DrawString(m_ChangeableRectStrs[8], m_BigDigitFont, Brushs.GreenBrush, m_ChangeableRects[8],
                FontInfo.SfRc);
        ...
        public override bool init(ref int nErrorObjectIndex)
        {
            ...
            m_PointX = new[] { 1, 45, 173, 217, 340, 384, 509 };
            m_PointY = new[] { 1, 52, 104 };
