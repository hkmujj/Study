...
namespace Subway.WuHanLine6.Converter
{
    ...
    /// 经济广播界面转换
    /// </summary>
    public class EmergencyBordercastConverter : IMultiValueConverter
    {
        /// <summary>
        /// 行
        /// </summary>
        ...
        {
            if (values == null || values.Any(a => a == null) || values.Any(a => a == DependencyProperty.UnsetValue) || parameter == null)
            {
                return DependencyProperty.UnsetValue;
            }

            var totalValue = (double)values[0];

            var isHeight = bool.Parse(parameter.ToString());

            var margin = (Thickness)values[1];

            var result = double.Epsilon;

            if (isHeight)
            {
                result = totalValue / Row - margin.Top - margin.Bottom;
            }
            else
            {
                result = totalValue / Colunm - margin.Left - margin.Right;
            }
