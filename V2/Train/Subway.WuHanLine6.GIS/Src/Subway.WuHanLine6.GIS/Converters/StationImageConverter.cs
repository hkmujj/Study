...
{
    public class StationImageConverter : IMultiValueConverter
    {
        /// <summary>
        ...
        {
            var staion = values[0] as StationName;
            if (staion == null)
            {
            ...
            else
            {
                if (staion.IsPast)
                {
                    return staion.IsTransfer ? values[3] : values[4];
                }
                else
                {
                    return staion.IsTransfer ? values[5] : values[6];
                }
            }
