...
namespace Motor.TCMS.CRH400BF.Converter
{
    public class SetValueConverter : IMultiValueConverter
    {
        ...
        public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            var satte = (TrainWorkConditionState)values[0];
            var tractionSetValue = (float)values[1];
            var brakeSetValut = (float)values[2];
            var height = (double)values[3];


            var tmpValue = 100 - ((satte == TrainWorkConditionState.Tow) ? tractionSetValue : brakeSetValut);

            return (tmpValue / 100 * height) - 50;
