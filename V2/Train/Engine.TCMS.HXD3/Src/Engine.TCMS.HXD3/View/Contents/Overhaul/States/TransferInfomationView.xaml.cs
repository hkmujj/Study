...
{
    /// <summary>
    /// TransferInfomationView.xaml 的交互逻辑
    /// </summary>
    [ViewExport(RegionName = RegionNames.RegionMainContentContent)]
    public partial class TransferInfomationView 
    {
        public TransferInfomationView()
        {
            InitializeComponent();
            ServiceLocator.Current.GetInstance<IEventAggregator>()
                ...
                .Subscribe((state) =>
                {
                    if (state.Id.ToString().Equals(StateKeys.Root_检修状态_状态_传送信息))
                    {
