...
namespace Engine.TCMS.HXD3.Model.BtnStragy
{
    ...
    public class StateInterfaceFactory : IStateInterfaceFactory
    {
        ...
        private void CreateStateInterface(StateInterfaceKey interfaceKey)
        {
            ...
            if (itConfig != null)
            {
                var state = new StateInterface
                {
                    ...
                    ContentViewName = string.Format("Engine.TCMS.HXD3.View.Contents.{0}", itConfig.ContentViewName),
        ...
        {
            var actionClassFullName = string.Format("Engine.TCMS.HXD3.Controller.BtnActionResponser.{0}", actionClassName);
            ...
            if (!string.IsNullOrWhiteSpace(actionClassName))
            {
                ...
                {
                    AppLog.Error("can not found action response where name = {0}", actionClassFullName);
                }
            }
