...
namespace LightRail.Ethiopia.TMS.Net
{
    ...
    public class V2_Net_C3_T : baseClass
    {
        #region 私有变量
        ...
        public override bool init(ref int nErrorObjectIndex)
        {
            ...
            string[] tmpFile0 = File.ReadAllLines(
                Path.Combine(AppConfig.AppPaths.ConfigDirectory,"T模块信息.txt"),
                Encoding.Default
                );
        ...
        {
            getValue();
            paint_T(dcGs, 43, -84, 5, new Rectangle(374, 190, 57, 25));
        ...
        /// </summary>
        /// <param name="dcGs"></param>
        private void paint_T(Graphics dcGs, float rowDistance, float columnDistance, Int32 rowCount, Rectangle rect)
        {
            dcGs.DrawLine(
                new Pen(new SolidBrush(Color.FromArgb(0, 255, 0)), 2),
                new PointF(rect.Left - (Math.Abs(columnDistance) - rect.Width), rect.Top + rect.Height / 2),
                new PointF(rect.Left + Math.Abs(columnDistance), rect.Top + rect.Height / 2)
                );//连接MC1与Mc2的线
