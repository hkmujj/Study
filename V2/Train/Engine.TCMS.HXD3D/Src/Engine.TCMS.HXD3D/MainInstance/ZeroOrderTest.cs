...
namespace Engine.TCMS.HXD3D.MainInstance
{
    ...
    internal class ZeroOrderTest : HXD3BaseClass
    {
        // Fields
        private readonly Image[] m_BaseImage = new Image[2];

        private readonly bool[] m_BtnIsDown = new bool[1];
        private readonly string[] m_BtnNames = { "主变流器CI1", "主变流器CI2", "主变流器CI3", "主变流器CI4", "主变流器CI5", "主变流器CI6", "结果" };
        ...
        private void InitData()
        {
            ...
            if (Coordinate.RectangleFLists(ViewIDName.ZeroOrderTest, ref m_RectsList))
            {
                m_BtnRegion.Add(new Region(m_RectsList[15]));
                m_HasPlan = true;
            }
        }
        ...
        {
            e.FillRectangle(new SolidBrush(Color.Blue), m_RectsList[0x10]);
            e.DrawString("确认", FontsItems.Fonts_Regular(FontName.宋体, 14f, false), SolidBrushsItems.WhiteBrush, m_RectsList[15], FontsItems.TheAlignment(FontRelated.居中));
            e.DrawImage(m_BtnIsDown[0] ? m_ImgsList[0] : m_ImgsList[1], m_RectsList[15]);
        }
        ...
        private void PaintValue(Graphics e)
        {
            ...
            for (num = 0; num < 6; num++)
            {
                e.DrawString(Convert.ToInt32(m_FValue[num]).ToString("#"), FontsItems.DefaultFont, SolidBrushsItems.WhiteBrush, m_RectsList[8 + num], FontsItems.TheAlignment(FontRelated.靠左));
            }
            e.DrawString("试验是否开始", FontsItems.Fonts_Regular(FontName.宋体, 14f, false), SolidBrushsItems.WhiteBrush, m_RectsList[14], FontsItems.TheAlignment(FontRelated.居中));
        }
    }
}