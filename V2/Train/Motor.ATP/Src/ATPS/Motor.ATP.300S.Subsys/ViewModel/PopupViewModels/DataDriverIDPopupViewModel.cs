            ...
            if (rlt.DriverInputType == DriverInputInterpreterResult.InputType.Control)
            {
                ...
                switch (word)
                {
                    case DriverInputControlWord.Ok:
                        DriverInterface.ATP.SendInterface.SendDriverId(new SendModel<DriverDataModel>(new DriverDataModel(null, DriverId)));
                        break;
                    case DriverInputControlWord.Cancel:
                        DriverInterface.ATP.SendInterface.SendTrainId(
                            new SendModel<DriverDataModel>(new DriverDataModel(null, DriverId), SendModelType.Cancel));
                        break;
