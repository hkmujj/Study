...
using Motor.ATP._300T.Subsys.Model.Message;
using Motor.ATP._300T.Subsys.ViewModel;

namespace Motor.ATP._300T.Subsys.Model
{
    public class ATP300TDomainModel : ATPDomain
    {
        public ATP300TDomainModel(SubsystemInitParam initParam):base(ATPType.ATP300T)
        {
            if (initParam != null)
            {
                InitParam = initParam;
        ...
        public override void Initalize(ScreenIdentity identity)
        {
            ...
            DriverInterfaceController = ServiceManager.GetService<IDriverInterfaceController>(ATPType.ATP300T.ToString());

            InfomationService = ServiceManager.GetService<IInfomationService>(ATPType.ATP300T.ToString());
            ...
            {
                new ATP300TPlanSectionCoordinate8K(),
                new ATP300TPlanSectionCoordinate16K(),
                new ATP300TPlanSectionCoordinate32K(),
            };
            ...
            Message = new ATP300TMessage(this, item => !item.Content.IsNullOrWhiteSpace());
            ...
            var speed = new Speed(TrainInfo) { SpeedDialPlate = new ATP300TSpeedDialPlate() };
            ...
            EmergencyInfo = new EmergencyInfo(this);
            CTCS = new CTCS(this);
                ...
                PropertySupport.ExtractPropertyName<ISpeedMonitoringSection, bool>(p => p.ZoomVisible))
            {
                if (!SpeedMonitoringSection.ZoomVisible)
                {
                    SpeedMonitoringSection.PlanSectionCoordinate =
                        PlanZoneCoordinates.Find(f => f.GetType() == typeof(ATP300TPlanSectionCoordinate32K));
                }
            }
        }
