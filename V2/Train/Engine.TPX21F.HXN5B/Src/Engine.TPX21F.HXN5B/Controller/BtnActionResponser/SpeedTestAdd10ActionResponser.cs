...
namespace Engine.TPX21F.HXN5B.Controller.BtnActionResponser
{
    ...
    public class SpeedTestAdd10ActionResponser : BtnActionResponserBase
    {
        ...
        {
            EventAggregator.GetEvent<SendDataRequestEvent>().Publish(new SendDataRequestEvent.Args(OubKeys.机车测试_速度测试_速度加10, true));
        }
        ...
        {
            EventAggregator.GetEvent<SendDataRequestEvent>().Publish(new SendDataRequestEvent.Args(OubKeys.机车测试_速度测试_速度加10, false));
        }
    }
}