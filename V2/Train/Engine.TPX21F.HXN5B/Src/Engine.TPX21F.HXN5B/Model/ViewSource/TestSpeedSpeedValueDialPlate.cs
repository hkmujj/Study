...
{
    public class TestSpeedSpeedValueDialPlate : IValueDialPlate
    {
        public static readonly TestSpeedSpeedValueDialPlate Instance = new TestSpeedSpeedValueDialPlate();

        public ReadOnlyCollection<IValueDialPlateDegree> AllValueDegrees { get; private set; }

        private TestSpeedSpeedValueDialPlate()
        {
        ...
        private IEnumerable<Tuple<int, int>> GetSpeedAngles()
        {
            var startAng = -45;
            var count = 24;
            float step = ((float) 180 + 45 - startAng)/count;
