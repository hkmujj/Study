...
{
    public class PortDataStyleToStringFormatConverter : IMultiValueConverter
    {
        ...
        {
            if (values != null && values != DependencyProperty.UnsetValue && values.Length >= 2 && values[1] is PortDataStyle && values[0] is int)
            {
                var d = (int)values[0];
                var sty = (PortDataStyle) values[1];

                switch (sty)
                {
                    case PortDataStyle.Base10:
                        return d.ToString();
                    case PortDataStyle.Base16:
                        return d.ToString("X");
                    default:
