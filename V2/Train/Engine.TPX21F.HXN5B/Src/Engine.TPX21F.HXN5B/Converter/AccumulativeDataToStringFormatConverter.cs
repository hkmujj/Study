...
{
    public class AccumulativeValueToStringConverter : IMultiValueConverter
    {
        ...
        {
            if (values != null && values != DependencyProperty.UnsetValue && values.Length >= 2 && values[0] is string &&
                values[1] is float)
            {
                var unit = (string) values[0];
                if (unit == "kW.h")
                {
                    return ((float) values[1]).ToString("0");
                }

                else
                {
                    return ((float) values[1]).ToString("0.0");
                }
            }
