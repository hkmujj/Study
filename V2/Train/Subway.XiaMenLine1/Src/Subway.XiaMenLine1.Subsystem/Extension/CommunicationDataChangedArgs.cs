...
namespace Subway.XiaMenLine1.Subsystem.Extension
{
    public static class CommunicationDataChangedArgsExtension
    {
        ...
        public static void SendData(this string logciName, bool value, bool isRest = false)
        {
            if (SubsysParams.Instance.IndexDescriptionConfig.OutBoolDescriptionDictionary.ContainsKey(logciName))
            {
                SubsysParams.Instance.SubsystemInitParam.CommunicationDataService.WriteService.ChangeBool(
                SubsysParams.Instance.IndexDescriptionConfig.OutBoolDescriptionDictionary[logciName], value);
                if (isRest)
                {
                    Task.Factory.StartNew(() =>
                    {
                        Thread.Sleep(200);
                        SubsysParams.Instance.SubsystemInitParam.CommunicationDataService.WriteService.ChangeBool(SubsysParams.Instance.IndexDescriptionConfig.OutBoolDescriptionDictionary[logciName], !value);
                    });
