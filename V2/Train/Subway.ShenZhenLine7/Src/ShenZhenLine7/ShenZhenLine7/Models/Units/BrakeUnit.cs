...
namespace Subway.ShenZhenLine7.Models.Units
{
    ...
    public class BrakeUnit : NotificationObject, ISetValueProvider, IUnit
    {
        ...
        public void DataChanged(IDictionary<int, bool> args)
        {
            ...
            args.UpdateIfContain(BrakeCut, b => State = m_BrakePriorityService.GetPriorityValue(BrakeState.BrakeCut, b, this));
            args.UpdateIfContain(BrakeFault, b => State = m_BrakePriorityService.GetPriorityValue(BrakeState.BrakeFault, b, this));
            args.UpdateIfContain(BrakeUnRemit, b => State = m_BrakePriorityService.GetPriorityValue(BrakeState.BrakeUnRemit, b, this));
            args.UpdateIfContain(BrakeRemit, b => State = m_BrakePriorityService.GetPriorityValue(BrakeState.BrakeRemit, b, this));
            args.UpdateIfContain(ParkingUnRemit, b => State = m_BrakePriorityService.GetPriorityValue(BrakeState.ParkingUnRemit, b, this));
            args.UpdateIfContain(ParkingRemit, b => State = m_BrakePriorityService.GetPriorityValue(BrakeState.ParkingRemit, b, this));
            args.UpdateIfContain(BrakeUnKnow, b => State = m_BrakePriorityService.GetPriorityValue(BrakeState.BrakeUnKnow, b, this));
        }
    }
