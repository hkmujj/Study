...
namespace Urban.GuiYang.DDU.Adapter.UpdateDataProviders
{
    ...
    public class CarMainPressureUpdateDataProvider : UpdateDataProviderBase
    {
        ...
        {
            var doors = ViewModel.TrainViewModel.Model.CarCollection.Select(s => s.Brake.BrakePage2.Value.MainPressure).Where(w => !string.IsNullOrWhiteSpace(w.ItemConfig.ValueIndex));
