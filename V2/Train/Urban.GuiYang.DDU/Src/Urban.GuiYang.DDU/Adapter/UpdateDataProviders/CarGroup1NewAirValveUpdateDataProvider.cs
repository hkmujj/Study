...
namespace Urban.GuiYang.DDU.Adapter.UpdateDataProviders
{
    ...
    public class CarGroup1NewAirValveUpdateDataProvider : UpdateDataProviderBase
    {
        [ImportingConstructor]
        public CarGroup1NewAirValveUpdateDataProvider(IEventAggregator eventAggregator, DomainViewModel viewModel)
        ...
        {
            var doors = ViewModel.TrainViewModel.Model.CarCollection.Select(s => s.AirCondition.AirConditionPage2.Value.Group1NewAirValve1).Where(w => !string.IsNullOrWhiteSpace(w.ItemConfig.Valve1UnkownIndex));
            var d = ViewModel.TrainViewModel.Model.CarCollection.Select(s => s.AirCondition.AirConditionPage2.Value.Group1NewAirValve2).Where(w => !string.IsNullOrWhiteSpace(w.ItemConfig.Valve2UnkownIndex));
        ...
        private NewAirValveState GetNewAirValveState1(CommunicationDataChangedArgs args, CarItem<NewAirValveState, CarGroup1NewAirValveConfig> it)
        {
        ...
        private NewAirValveState GetNewAirValveState2(CommunicationDataChangedArgs args, CarItem<NewAirValveState, CarGroup1NewAirValveConfig> it)
        {
